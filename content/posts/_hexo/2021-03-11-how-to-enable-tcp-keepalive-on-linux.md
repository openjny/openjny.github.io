...
なお、TCP の状態遷移において、時間管理が必要な場面は多々あります。例えば、FIN を送信してからしばらく `TIME_WAIT` 状態でコネクションを待つ[^time-wait]のですが、ここでも時間管理は必要です。したがって、timer フィールドがあるからといって TCP KeepAlive が有効化されているとは限らないので注意してください。
